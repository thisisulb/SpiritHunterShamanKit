...
**Warnings and Notes:**  
**-This kit is build to be used by player1 (the protagonist in a single player game, first slot in multi-player sessions) only, using it for other party members will result in bugs/not work properly)**
**  
...
This spell imbues the caster's bow with a frost magic spell for up to one turn. The spell will travel with any arrow released by the caster's bow but only take effect once, on the first successful hit made during its duration.
